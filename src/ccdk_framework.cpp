...
static int ccdk_framework_sync_wait_response_condition(struct ccdk_framework_condition *condition,
        int timeout);
...
static inline int
ccdk_framework_condition_signal(struct ccdk_framework_condition *condition)
{
    pthread_mutex_lock(&condition->cv_mutex);
    rte_atomic32_set(&condition->cv_signalled, CCDK_SESSION_SIGNALLED);
    pthread_cond_signal(&condition->cv);
    pthread_mutex_unlock(&condition->cv_mutex);
    FRAMEWORK_DEBUG("\n");
    return EC_CCDK_OK;
}
...
ccdk_framework_free_session(uint32_t session_id)
{
    ...
    pthread_mutex_destroy(&session->cond.cv_mutex);
    pthread_cond_destroy(&session->cond.cv);
                           ...
                           uint32_t *session_id)
{
    ...
    pthread_mutex_init(&session->cond.cv_mutex, NULL);
    pthread_cond_init(&session->cond.cv, NULL);
    rte_atomic32_set(&session->cond.cv_signalled, 0);
...
ccdk_framework_get_server(const char *name, const uint32_t server_id)
{
    ...
    for (int i = 0; i < CCDK_MAX_SERVER_NUM; i++) {
        ...
        if (server->status == CCDK_UNUSED) {
            ...
            pthread_mutex_init(&server->cond.cv_mutex, NULL);
            pthread_cond_init(&server->cond.cv, NULL);
            rte_atomic32_set(&server->cond.cv_signalled, 0);
...
ccdk_framework_set_msg_done(void *msg, int result)
{
    ...
    if (req->front_is_async) {
        if (session->is_local) {
            ret = ccdk_framework_condition_signal(&g_framework.cond);
        } else {
            ret = ccdk_framework_eventfd_signal(session->event_fd[CCDK_SESSION_ASYNC_EVENTFD]);
        }
    } else {
        if (session->is_local) {
            ret = ccdk_framework_condition_signal(&session->cond);
        } else {
...
static int
ccdk_framework_server_condition_mode(struct ccdk_framework_server *server)
{
    return ccdk_framework_sync_wait_response_condition(&server->cond, 3000);
}
...
ccdk_framework_server_thread_main(void *arg)
{
    ...
    while (server->thread_running) {
        ...
        if (unlikely(ccdk_framework_server_condition_mode(server)) < EC_CCDK_OK) {
            break;
        }
...
static int ccdk_framework_sync_wait_response_eventfd(int epfd, int maxevents, int timeout)
{
    int n = 0;
        ...
        int timeout)
{
    ...
    outtime.tv_nsec = now.tv_usec * 1000 + timeout * 1000000;
    FRAMEWORK_DEBUG("\n");
    pthread_mutex_lock(&condition->cv_mutex);

    while (rte_atomic32_read(&condition->cv_signalled) != CCDK_SESSION_SIGNALLED) {
        //pthread_cond_timedwait(&condition->cv, &condition->cv_mutex, &outtime);
        pthread_cond_wait(&condition->cv, &condition->cv_mutex);
    }

    rte_atomic32_set(&condition->cv_signalled, 0);
    pthread_mutex_unlock(&condition->cv_mutex);
    FRAMEWORK_DEBUG("\n");
    return EC_CCDK_OK;
}
...
static int ccdk_framework_async_wait_response_condition(struct ccdk_framework_condition *condition,
        int timeout)
{
    int ret = 0, n = 0;
    struct ccdk_framework_req *req, *next;
    ret = ccdk_framework_sync_wait_response_condition(condition, timeout);

    if (ret < EC_CCDK_OK) {
        FRAMEWORK_ERR("ccdk_framework_sync_wait_response_condition failed, ret = %d", ret);
        return ret;
    }

    for (int i = 0; i < CCDK_MAX_SESSION_NUM; i++) {
        struct ccdk_framework_session *session = (struct ccdk_framework_session *)&g_framework.session[i];
...
static int ccdk_framework_async_wait_response_eventfd(int epfd, int maxevents, int timeout)
{
    int ret = 0, n = 0;
    struct ccdk_framework_req *req, *next;
    struct rte_epoll_event revent[CCDK_MAX_SESSION_NUM] = {};
...
ccdk_framework_local_sync_handle_request(uint32_t session_id, uint32_t server_id, void *msg, int timeout)
{
    ...
    if (ret > 0) {
        ...
        if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
            ret = ccdk_framework_sync_wait_response_condition(&session->cond, timeout);
        } else {
            ret = ccdk_framework_sync_wait_response_eventfd(session->epfd, 1, timeout);
        }
...
ccdk_framework_send_request(int is_async, uint32_t session_id, uint32_t server_id, void *msg, int timeout)
{
    ...
    if (server) {
        ...
        if (!is_async) {
            /// @brief trigger
            if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
                ret = ccdk_framework_condition_signal(&server->cond);
            } else {
...
ccdk_framework_mempool_get_msg(int size)
{
    ...
    if (unlikely(!request_pool)) {
        ...
        if (!request_pool) {
            ...
            if (unlikely(!request_pool)) {
                request_pool = rte_mempool_create(namebuf, 1024,
...
ccdk_framework_cfg_mp_primary(const struct rte_mp_msg *msg, const void *peer)
{
    ...
    case SOCKET_CCDK_REQ_SESSION_EVENT_FD: {
        ...
        if (session_m->session_id < CCDK_MAX_SESSION_NUM) {
            r->param.req = SOCKET_CCDK_REQ_SESSION_EVENT_FD;
            r->param.result = 0;
...
ccdk_framework_init_primary(void)
{
    int ret;
#ifdef EVENTFD_MODE
    ret = rte_mp_action_register(CCDK_CFG_MP, ccdk_framework_cfg_mp_primary);
    ...
    ret = pthread_create(&g_framework.thread, NULL, ccdk_framework_primary_thread_main, &g_framework);
    (void)rte_thread_setname(g_framework.thread, "primary");
...
ccdk_framework_init_secondary(void)
{
    ...
    ret = pthread_create(&g_framework.thread, NULL, ccdk_framework_secondary_thread_main, &g_framework);
    (void)rte_thread_setname(g_framework.thread, "secondary");

    if (ret < 0) {
        FRAMEWORK_ERR("Cannot create secondary thread, ret = %d\n", ret);
        return EC_CCDK_INIT_FAIL;
    }
...
ccdk_framework_response_thread_main(void *arg)
{
    ...
    while (g_framework.resp_thread_running) {
        ...
        if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
            ret = ccdk_framework_async_wait_response_condition(&g_framework.cond, 3000);
        } else {
            ret = ccdk_framework_async_wait_response_eventfd(g_framework.session_epfd, CCDK_MAX_SESSION_NUM, 3000);
        }
